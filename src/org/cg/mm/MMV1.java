...
public class MMV1 {
   ...
   static class DestroyerSkillEffect extends SkillEffect {
      ...
      DestroyerSkillEffect(int type, int x, int y, double radius, int duration, int order,
            Destroyer destroyer) {
   ...
   static class ReaperSkillEffect extends SkillEffect {
      ReaperSkillEffect(int type, int x, int y, double radius, int duration, int order,
            Reaper reaper) {
   ...
   static class Wreck extends Point {
      ...
      public Wreck midWay(Wreck wreck, double ratio) {
         ...
         return new Wreck((int) (this.x + dx * ratio), (int) (this.y + dy * ratio), this.water,
               this.radius);
      }
   }
   ...
   private static Wreck findNearestWreck(List<Reaper> reapers, List<Destroyer> destroyers,
         List<Doof> doofs, List<Wreck> wrecks, List<Tanker> tankers,
   ...
   private static Destroyer findNearPotentialDestroyer(Reaper reaper, List<Destroyer> destroyers,
         List<Tanker> tankers, boolean meAttacking) {
   ...
   private static Tanker findTanker(List<Destroyer> destroyers, List<Tanker> tankers,
         List<Reaper> reapers, boolean meAttacking) {
   ...
   public static void main(String args[]) {
      ...
      while (true) {
         ...
         for (int i = 0; i < unitCount; i++) {
            ...
            if (unitType == TYPE_WRECK) {
               ...
               Tanker tanker = new Tanker(
                     (int) ((radius - TANKER_RADIUS_BASE) / TANKER_RADIUS_BY_SIZE), playerO);
               tanker.id = unitId;
               tanker.mass = mass;
               ...
               System.err.println("4 unitId = " + unitId + " unitType = " + unitType + " player = "
                     + player + " mass = " + mass + " radius = " + radius + " x = " + x + " y = "
                     + y + " vx = " + vx + " vy = " + vy + " extra = " + extra + " extra2 = "
                     + extra2);
            }
         }
         ...
         Tanker tankerR = findTanker(destroyers, tankers, reapers,
               myScore > counter / COUNTER_DIVIDER);

         wrecks = addOverlapping(wrecks);
         Wreck selWreck = findNearestWreck(reapers, destroyers, doofs, wrecks, tankers,
               skillEffects);
         // Random r = new Random();

         if (selWreck != null) {
            System.out.println(
                  selWreck.x - reaper.vx + " " + (selWreck.y - reaper.vy) + " " + MAX_THRUST);
         } else {
            ...
            if (tankerR != null) {
               ...
               System.out.println(
                     otherDest.x - reaper.vx + " " + (otherDest.y - reaper.vy) + " " + MAX_THRUST);
            } else {
               if (doof1.distance(reaper) < doof2.distance(reaper)
                     && enemyRage1 > DOOF_SKILL_COST) {
                  System.out.println(
                        doof1.x - reaper.vx + " " + (doof1.y - reaper.vy) + " " + MAX_THRUST);
               } else if (enemyRage2 > DOOF_SKILL_COST) {
                  System.out.println(
                        doof2.x - reaper.vx + " " + (doof2.y - reaper.vy) + " " + MAX_THRUST);
               } else {
                  System.out.println(WATERTOWN.x - reaper.vx + " " + (WATERTOWN.y - reaper.vy) + " "
                        + MAX_THRUST);
               }
            }
         }
         ...
         if (enemyWreck != null && enemyWreck.isInRange(destroyer, DESTROYER_SKILL_RANGE)) {
            System.out.println(enemyWreck.x + destroyer.vx + " " + (enemyWreck.y + destroyer.vy)
                  + " " + MAX_THRUST);
         } else if (enemyWreck != null
               && enemy.isInRange(enemyWreck, enemyWreck.radius + enemy.radius)) {
         ...
         if (enemyWreck != null && enemyWreck.isInRange(doof, DOOF_SKILL_RANGE)
            ...
            doofSkillCountDown = DOOF_SKILL_DURATION;
               ...
               && !reaper.isInRange(otherEnemyWreck, otherEnemyWreck.radius)
               && myRage > DOOF_SKILL_COST && doofSkillCountDown == 0 && isOtherEnemyAhead) {
